...
Note that we considered two other approaches for updating the mean weight index that we do not recommend and are not presented here. The first was a multiple imputation approach, in which a state space AR1 model was fit to the observed mean weight index (1997-2017), then the fitted model was used to impute mean weight values for the missing years (2018-2020). The correct method for multiple imputation [@rubin1987], requires multiple (*x*) replicates, where: i) *x* new mean weight indices are imputed; ii) the *x* indices are input into *x* assessment models; and iii) the *x* sets of posterior model estimates (biomass and recruitment) are pooled using a model averaging approach. While this approach yielded results with low bias, it is intractable for use in the 2022 assessment update because the advice is already based on model-averaged results from seven sensitivity cases. Therefore to apply this approach, we would have to run 7*x* versions of the model for each stock (i.e., each of the seven sensitivity cases would itself be the result of averaging *x* models). This is intractable in terms of computing and analyst time and also represents a change to the assessment methodology that is too large for an update. The second approach we considered was calibrating a survey mean weight index to the commercial mean weight index, and using a calibrated survey mean weight index going forward. This is problematic for two main reasons: i) survey lengths are collected using a length-stratified approach and are not representative of the length distribution in the population; and ii) it is not clear how differences between commercial and survey selectivity would be accounted for in the calibration.  

Also note that, while the analyses presented here can provide some guidance on the possible relative impacts of  alternative approaches for treating the loss of commercial length samples, the true impact of missing commercial length data from 2020 to 2022 can never be fully evaluated, as these data do not exist. The direction of any assessment biases arising from different treatments of the mean weight index will depend on the magnitude of differences between the last real length observations and the true, unknown, lengths in the population. Therefore, compared to the exploratory 2020 results presented here, the magnitude and even the direction of biases may differ in the 2022 assessment update.

# Methods

For each of Area 3CD and Area 5ABCD, we evaluated three alternative treatments of the commercial mean weight index, *assuming that no commercial length data had been available to calculate a mean weight index for 2018-2020*. For each scenario, we compared estimated mean weight, biomass and recruitment to estimates from the Reference case model of the 2020 assessment [@dfo2021], which used all available values of the mean weight index. We also show impacts on the forecasted 2021 biomass. Except for the treatment of the final three years of the commercial mean weight index (2018-2020), all other model settings were identical to the Reference case models.

The three treatments (scenarios) were:

Sc 1. *No commercial mean weight data for 2018-2020*. For Area 3CD, this meant there was no commercial mean weight index for the last four years of the assessment because there were already no values for 2017 and 2018 (Table \@ref(tab:tab-mean-weight-recent-3cd)). For Area 5ABCD, this meant no commercial mean weight index for the last three years of the assessment (Table \@ref(tab:tab-mean-weight-recent-5abcd)).

Sc 2. *Use the last observed mean weight index value for 2018-2020*. For Area 3CD, this meant using the 2016 commercial mean weight index value for model years 2018-2020 (Table \@ref(tab:tab-mean-weight-recent-3cd)). This actually meant adding a data point for 2018 because there was no commercial sampling that year (and therefore no 2018 index point had been used in the Reference model). For Area 5ABCD, this meant using the 2017 commercial mean weight index value for model years 2018-2020 (Table \@ref(tab:tab-mean-weight-recent-5abcd)). 

Sc 3. *Use a 3-year average mean weight observation for 2018-2020*. For Area 3CD, this meant using the 2014-2016 average of the commercial mean weight index for model years 2018-2020 (Table \@ref(tab:tab-mean-weight-recent-3cd)). As for Scenario 2, this meant adding a data point for 2018. For Area 5ABCD, this meant using the 2015-2017 average of the commercial mean weight index for model years 2018-2020 (Table \@ref(tab:tab-mean-weight-recent-5abcd)).
...
```{r fig-base-mean-weight-3cd-sc1, fig.cap="Scenario 1 model MPD fit to the mean weight data for Area 3CD (no data for 2018-2020)."}
...
```{r fig-base-mean-weight-3cd-sc2, fig.cap="Scenario 2 model MPD fit to the mean weight data for Area 3CD (2016 mean weight index value used for 2018-2020)."}
...
```{r fig-base-mean-weight-3cd-sc3, fig.cap="Scenario 3 model MPD fit to the mean weight data for Area 3CD (mean 2014:2016 index value used for 2018-2020)."}
...
```{r fig-base-mean-weight-5abcd-sc1, fig.cap="Scenario 1 model MPD fit to the mean weight data for Area 5ABCD (no data for 2018-2020)."}
...
```{r fig-base-mean-weight-5abcd-sc2, fig.cap="Scenario 2 model MPD fit to the mean weight data for Area 5ABCD (2017 mean weight index value used for 2018-2020)."}
...
```{r fig-base-mean-weight-5abcd-sc3, fig.cap="Scenario 3 model MPD fit to the mean weight data for Area 5ABCD (mean 2015:2017 index value used for 2018-2020)."}
