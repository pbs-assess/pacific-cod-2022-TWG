...
We used a simple GLM with a Gamma log link function, to estimate model fit of log  commercial mean weights ($y_t$) in year $t$ to log survey mean weights ($W_t$) from Step 1:
...
We then used the `predict` function in R to predict commercial mean weights from survey mean weights. Note that we included the 2021 survey indices for both areas in the regression model.
...
For **Area 3CD**, there was a WCVI survey in 2018, but no survey in 2020. Therefore, for the 2018-2020 period of interest, there was only a predicted commercial mean weight index point in 2018. We ran two alternative scenarios. **Scenario 1:** values for 2019 and 2020 were obtained by linear interpolation between the 2018 and 2021 predicted values; and **Scenario 2:** we used only the 2018 value. We used the `approx` function in R for the interpolation, in natural space.
